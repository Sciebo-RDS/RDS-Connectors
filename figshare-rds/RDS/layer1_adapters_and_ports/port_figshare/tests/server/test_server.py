...
api_key = os.getenv("FIGSHARE_API_KEY", default=None)
...
class TestPortFigshare(unittest.TestCase):
    def setUp(self):
        self.app = create_app()
    ...
    def test_home_status_code_json(self):
        ...
        admintoken = {
            ...
            "data": {
                ...
                "user": user,
                "service": {"data": {"servicename": "port-figshare"}},
            },
        }
        ...
        ).with_request("GET", "/user/admin/service/port-figshare").will_respond_with(
            200, body=admintoken
        )

        pact.given("user admin exists in figshare").upon_receiving(
            "user has no deposit"
        ...
        result = None
        with pact:
            data = {"userId": "figshare://user:ASD123GANZSICHA"}
    ...
    def test_without_apikey(self):
        ...
        pact.given("user admin not exists in figshare").upon_receiving(
            "user has no deposit"
        ).with_request("GET", "/api/deposit/depositions").will_respond_with(
            401, body=expected
        )
    ...
    def test_projectId_not_found(self):
        ...
        pact.given("projectId not exists in figshare").upon_receiving(
            "user has no deposit"
        ).with_request(
    ...
    def test_index_metadata(self):
        ...
        expected_body = {
            ...
            "metadata": {
                "prereserve_doi": {"doi": "10.5072/figshare.1234", "recid": projectId}
            },
        ...
        expected_body = {
            ...
            "metadata": {
                ...
                "image_type": "drawing",
                "license": "CC-BY-4.0",
                "prereserve_doi": {"doi": "10.5072/figshare.662835", "recid": 662835},
    ...
    def test_get_metadata(self):
        ...
        expected_body = {
            ...
            "metadata": {
                ...
                "metadata": {
                    "prereserve_doi": {"doi": "10.5072/figshare.1234", "recid": 1234}
                },
                "modified": "2016-06-15T16:10:03.319371+00:00",
                "owner": 1,
                "record_id": 1234,
    ...
    def test_patch_metadata(self):
        ...
        updated_metadata = {
            ...
            "creators": [{"name": "Doe, John", "affiliation": "Figshare"}],
        }

        result_metadata = {
            "https://schema.org/creator": [
                {
                    "https://schema.org/affiliation": "Figshare",
            ...
            "https://www.research-data-services.org/jsonld/figsharecategory": "poster",
        ...
        with pact:
            data = {"userId": "figshare://user:ASD123GANZSICHA", "metadata": {"title": ""}}
            result = self.client.patch(f"/metadata/project/{projectId}", json=data)
            self.assertEqual(result.status_code, 200)
            self.assertEqual(result.json, result_metadata)
    ...
    def test_create_metadata(self):
        ...
        with pact:
            data = {"userId": "figshare://user:ASD123GANZSICHA", "metadata": updated_metadata}
            result = self.client.post("/metadata/project", json=data)
            self.assertEqual(result.status_code, 200)
    ...
    def test_metadata_get_jsonld_missing(self):
        ...
        # should return figshare specific api dataset, because there was an error in transfomration
        with pact:
            data = {"userId": "figshare://user:ASD123GANZSICHA"}
            result = self.client.get(f"/metadata/project/{projectId}", json=data)
            self.assertEqual(result.status_code, 200)
    ...
    def test_metadata_update_jsonld_complete(self):
        ...
        with pact:
            data = {"metadata": metadata, "userId": "figshare://user:ASD123GANZSICHA"}
            result = self.client.patch(f"/metadata/project/{projectId}", json=data)
            self.assertEqual(result.status_code, 200)
            self.assertEqual(result.json, expected_body["metadata"])
